...
public class PartesBD {
    private final String RUTA_ARCHIVO = System.getProperty("user.dir")+ "\\src\\Archivo\\Partes.txt";
    ...
    private static PartesBD instance = null;
    ...
    public static PartesBD getInstance() {
        if (instance == null) {
            instance = new PartesBD();
        }
        return instance;
    }
     ...
     * @return ArrayList
     */
     public ArrayList<Partes> listaPartes(  ) throws Exception{
      ...
      try {         
            ...
            while(true){
                Partes vehiculo1 = (Partes)oLector.readObject();                
                listaPartes.add(vehiculo1);
            }
    ...
    public Partes consultarPartes(String codigo)throws Exception {
        Partes parte,parteBuscada= null ;
        try {
            abrirArchivoInput();
            //Si no hay más datos que leer el método available retorna cero
             while(true){
                parte = (Partes)oLector.readObject();  //Lee un objeto a la vez  
              
                if(parte.getCodigo().equals(codigo)) {
                    parteBuscada = parte;
                }
             }            
        }catch(Exception e){
     ...
     * @return void
     */
    public  void agregarParte(Partes partes)throws Exception {        
        try {
            this.abrirArchivoOutput(); //se coloca al final del archivo
            if (oEscritor != null) {
              //Ejecutar la escritura en el archivo
               oEscritor.writeObject(partes); //Serializa el objeto y lo graba en el archivo
               oEscritor.flush();  //Envía el contenido del buffer al archivo
               oEscritor.reset();//Se requiere para cuando se reciben subclases , es obligatorio si hay herencia
            }
     ...
     * @return void
     */
      public void modificarParte(Partes partes) throws Exception{
        ...
        try {            
            ...
            //Si no hay más datos que leer el método available retorna cero
             while(true){//Si va a leer y no hay objeto se va por el catch
                 partes1 = (Partes)oLector.readObject(); //Si lee y no hay que leer se cae
                 if(partes1.getCodigo().equalsIgnoreCase(partes.getCodigo())) {
                   partes1=partes;
                 }
                 arrayPartesTemp.add(partes1);
             }  
        }
        catch(Exception ex){
    ...
    public void eliminarParte(String codigo) throws Exception {
        arrayPartesTemp = new ArrayList<Partes>();
        try {            
            abrirArchivoInput();
            Partes Partes1 = null;
            ...
            while(true){
                 Partes1 = (Partes)oLector.readObject();               
                 if(!Partes1.getCodigo().equalsIgnoreCase(codigo)) {
                     arrayPartesTemp.add(Partes1);
                 }
             }                       
        }catch(Exception e){      
    ...
    private void pasarArrayTemporal_Archivo() throws Exception {       
       ...
       //borra el archivo original para volverlo a llenar           
          ...
          //Pasa todos los  del ArrayList al archivo
            for (Partes partes : arrayPartesTemp) {
             oEscritor.writeObject(partes);   
